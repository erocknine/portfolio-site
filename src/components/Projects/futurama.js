...
const FuturamaDoc = (props) => {
  const { openDoc, handleProject } = props;

  return (
    <div className={openDoc === 'futurama' ? `projects-document futurama`: "projects-document"}>
      <div id="futurama" className="project-title">
        <p>Technical</p>
        <h1>Futurama</h1>
        <p className="text">October 2020</p>
        <p className="text">A site dedicated to Futurama with over 100 memorable quotes and over 50 Futurama images!</p>
        <a href="https://futuramaquotes.netlify.app/" target="_blank" rel="noopener noreferrer">View Futurama Quotes</a>
      </div>
      ...
      <div className="project-content">
        ...
        <div className={ openDoc === 'futurama' ? "project-portion":"project-portion closed" }>
          <p className="p-solo">I love Futurama. I've watched this show since it first came out in the 2000's after the fright of Y2K, and since then I've always referenced quotes and laughed in my head whenever I recalled hilarious moments. I'm positive many others feel the same, so I wanted to dedicate a site to all the funny, memorable quotes.</p>
        </div>
        ...
        <div className={ openDoc === 'futurama' ? "project-portion":"project-portion closed" }>
          <p className="p-solo">To anyone who's seen the show, the intro animation is iconic. Upon closer inspection, I realized the colors and lights in the original show was actually numerous spotlights glaring up at the sky and at the iconic 'FUTURAMA' title.</p>
        </div>

        <div className={ openDoc === 'futurama' ? "project-portion":"project-portion closed" }>
          <p className="p-solo">I thought I would recreate this using pure CSS. I created numerous divs of light & dark blue colors with varying widths, all originating from the bottom center point of the screen. These divs would then rotate side-to-side from this focal point like a panning spotlight using CSS animations.</p>
        </div>
        ...
        <div className={ openDoc === 'futurama' ? "project-portion":"project-portion closed" }>
          <p className="p-solo">A few CSS transitioning tricks later and the user is taken to the main page. Here, a quote is automatically loaded on the bottom with a 'NEXT' button, the quote's episode name and number on the top left corner, and an 'IMAGES' button.</p>
        </div>

        <div className={ openDoc === 'futurama' ? "project-portion":"project-portion closed" }>
          <p className="p-solo">There are over 100 quotes included in this application, each of them a separate object with keys 'speaker', 'quote', 'episode', and 'episode_code', all manually compiled by yours truly. I scraped websites and placed all of these objects into a large 'quotes' array to be rendered from.</p>
        </div>
        ...
        <div className={ openDoc === 'futurama' ? "project-portion-stacked":"project-portion closed" }>
          <p className="p-solo">On the opposite top right corner is an 'IMAGES' button, which activates several things. First, the main title 'FUTURAMA' is changed to a CSS class of a smaller size in order to make room for the new element of randomly rendered images. The source of these images sit in a large array, and their indexes randomly called from an element utilizing Math.rand(). There are over 50 images manually curated for presenting, changing every 5 seconds to a new randomly selected image.</p>
          <p className="p-solo">From the main page, the episode name and code is rendered onto the top left corner. Clicking this element also animates the Planet Express Ship flying across the screen!</p>
        </div>
